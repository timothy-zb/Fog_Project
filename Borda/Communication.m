...
% Find the indices of the nodes with the highest sum of occurrences
[~, detectedNodes] = sort(occurrencesSum, 'descend');
detectedNodes = detectedNodes(1:numMalicious);  % Select top 'numMalicious' nodes

% Display the actual malicious nodes
disp('Actual Malicious Nodes:');
disp(maliciousNodes);

% Display the detected nodes
disp('Detected Malicious Nodes:');
disp(detectedNodes);

% Compare detected nodes with actual malicious nodes
correctlyDetected = intersect(maliciousNodes, detectedNodes);
disp('Correctly Detected Malicious Nodes:');
disp(correctlyDetected);

% Calculate and display the number of correctly detected malicious nodes
numCorrectlyDetected = numel(correctlyDetected);
disp(['Number of Correctly Detected Malicious Nodes: ', num2str(numCorrectlyDetected)]);
